...
// Purchase with wallet - FIXED ORDER CREATION
app.post('/api/purchase-data', requireAuth, async (req, res) => {
    ...
    try {
        ...
        // ✅ FIX 1: ALWAYS CREATE ORDER RECORD FIRST (with 'processing' status)
        ...
        const initialOrderData = {
            ...
            reference: reference,
            transactionId: null, // Will be updated when Hubnet responds
            hubnetResponse: null, // Will be updated
        ...
        console.log('✅ Order record created in Firebase with processing status');
        ...
        // ✅ FIX 2: UPDATE ORDER BASED ON HUBNET RESPONSE
        ...
        // ✅ FIX 3: UPDATE ORDER STATUS ON ANY ERROR
        if (transactionRef) {
            await transactionRef.update({
                status: 'failed',
            ...
            console.log('✅ Order updated to failed due to system error');
        }
...
app.post('/api/hubnet-webhook', webhookLimiter, async (req, res) => {
    ...
    try {
        ...
        let update = { 
            ...
            hubnetWebhookData: req.body // Store full webhook data
        };
...
// Process direct purchase - FIXED userData issue
app.get('/api/process-direct-purchase/:reference', requireAuth, async (req, res) => {
    ...
    try {
        ...
        if (result.data.status === 'success') {
            ...
            // ✅ IMPROVED VOLUME CONVERSION
            let volumeValue = '1000'; // default
            if (packageName) {
                const volumeMatch = packageName.match(/\d+/);
                if (volumeMatch) {
                    const volumeNumber = parseInt(volumeMatch[0]);
                    // Convert GB to MB if it's a small number (assume < 100 is GB)
                    volumeValue = volumeNumber < 100 ? 
                        (volumeNumber * 1000).toString() : 
                        volumeNumber.toString();
                    console.log(`🔢 DIRECT PURCHASE CONVERTED: ${volumeNumber} → ${volumeValue}MB`);
                }
            }

            // ✅ CREATE ORDER RECORD FIRST (just like wallet purchase)
            transactionRef = admin.database().ref('transactions').push();
            const initialOrderData = {
                userId,
                network,
                ...
                fee: (result.data.amount / 100) - originalAmount,
                status: 'processing',
                reference: reference,
                transactionId: null,
                hubnetResponse: null,
            ...
            await transactionRef.set(initialOrderData);
            console.log('✅ Direct purchase order created in Firebase');
            ...
            const hubnetResponse = await axios.post(
                `https://console.hubnet.app/live/api/context/business/transaction/${network}-new-transaction`,
                {
                    ...
                    volume: volumeValue,
                    reference: reference,
                    referrer: userData.phone || '', // ✅ NOW FIXED: userData is defined
                    webhook: `${process.env.BASE_URL}/api/hubnet-webhook`
                },
                {
                    headers: {
            ...
            if (hubnetData.status === true && hubnetData.code === "0000") {
                ...
                console.log('❌ Direct purchase failed at Hubnet');

                // Don't throw error, return proper response
                res.status(400).json({ 
                    success: false, 
                    error: hubnetData.reason || `Hubnet error: ${hubnetData.code}`,
                    details: 'Hubnet purchase failed'
                });
            }
        } else {
            throw new Error('Payment verification failed: ' + (result.data.gateway_response || 'Unknown error'));
        }
    } catch (error) {
...
// Fix the user role update endpoint
...
app.get('/api/admin/users/roles', requireAdmin, async (req, res) => {
    try {
        const usersSnapshot = await admin.database().ref('users').once('value');
        const users = usersSnapshot.val() || {};
        
        const usersWithRoles = Object.entries(users).map(([uid, userData]) => ({
            uid,
...
// Fix for package management - get all packages with proper structure
app.get('/api/admin/packages/all', requireAdmin, async (req, res) => {
    try {
        const packagesSnapshot = await admin.database().ref('packages').once('value');
        const packages = packagesSnapshot.val() || {};
        
        // Transform the package structure for the admin panel
        const formattedPackages = {
            mtn: Object.entries(packages.mtn || {}).map(([id, pkg]) => ({
                id: id,
                name: pkg.name,
                price: pkg.price,
            ...
            at: Object.entries(packages.at || {}).map(([id, pkg]) => ({
                id: id,
                name: pkg.name,
                price: pkg.price,
                validity: pkg.validity,
